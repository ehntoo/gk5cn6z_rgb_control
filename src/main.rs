...
#[rustfmt::skip]
fn _adjust_color(color: Color) -> Color {
    match color {
...
fn main() {
    ...
    kb.send_feature_report(&refresh_report)
        .expect("Failed to send refresh report");

    let set_picture_report = [
        0u8,
        FeatureReportOpcode::SetPicture as u8,
        0,
        0,
        8,
        0,
    ...
    kb.send_feature_report(&set_picture_report)
        .expect("Failed to send set picture report");
